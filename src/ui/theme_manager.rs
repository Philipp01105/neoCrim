...
impl ThemeManager {
    pub fn new() -> Self {
        ...
        embedded_themes.insert("dark".to_string(), include_str!("../../themes/dark.nctheme"));
        embedded_themes.insert("light".to_string(), include_str!("../../themes/light.nctheme"));
        embedded_themes.insert("monokai".to_string(), include_str!("../../themes/monokai.nctheme"));
        embedded_themes.insert("glass_dark".to_string(), include_str!("../../themes/glass_dark.nctheme"));
        embedded_themes.insert("cyberpunk".to_string(), include_str!("../../themes/cyberpunk.nctheme"));
        embedded_themes.insert("dracula".to_string(), include_str!("../../themes/dracula.nctheme"));
        embedded_themes.insert("nord".to_string(), include_str!("../../themes/nord.nctheme"));
        embedded_themes.insert("solarized_dark".to_string(), include_str!("../../themes/solarized_dark.nctheme"));
    ...
    pub fn get_theme_by_name(&self, name: &str) -> Result<NeoTheme> {
        let theme_content = self.embedded_themes.get(name)
    ...
    pub fn get_theme_by_index(&self, index: usize) -> Result<NeoTheme> {
        if index >= self.theme_names.len() {
            return Err(anyhow::anyhow!("Theme index {} out of range (0-{})", index, self.theme_names.len() - 1));
        }
    ...
    pub fn get_theme_index(&self, name: &str) -> Option<usize> {
        self.theme_names.iter().position(|theme_name| theme_name == name)
    }
